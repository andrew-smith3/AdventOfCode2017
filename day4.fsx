...
open System.Web.UI.WebControls.Adapters
...
let validatePhrase (phrase: string) = 
    let words = phrase.Split [|' '|]
    let rec recCheck (usedWords: string list) (words: string list) = 
        match words with 
        | [] -> true
        | _ when List.contains words.Head usedWords -> false
        | head::tail -> 
            let usedWords = head::usedWords
            recCheck usedWords tail
    recCheck [] (Array.toList words)

let main (input: string) = 
    input.Split [|'\n'|]
    |> Array.map validatePhrase
    |> Array.filter id
    |> Array.length

let areAnagrams (x: string) (y: string) = 
    let sortedX = x.ToCharArray() |> Array.sort
    let sortedY = y.ToCharArray() |> Array.sort
    sortedX = sortedY

let rec wordValidForPhrase (word: string) (words:string list) = 
    match words with 
    | [] -> true
    | head::tail when areAnagrams word head -> false
    | head::tail -> wordValidForPhrase word tail

let validateAnagram (phrase: string) = 
    let words = phrase.Split [|' '|] |> Array.toList
    let rec helper word words = 
        match wordValidForPhrase word words with
        | false -> false
        | true when words.IsEmpty -> true
        | true -> helper words.Head words.Tail

    helper words.Head words.Tail

let main2 (input: string) = 
    input.Split [|'\n'|]
    |> Array.map validateAnagram
    |> Array.filter id
    |> Array.length


let finalTest = "oaoe rxeq vssdqtu xrk cjv yaoqp loo
