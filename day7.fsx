...
let (|Weight|_|) (input: string) =
    let toParse = input.Split([|'('; ')'|], StringSplitOptions.RemoveEmptyEntries) |> String.concat ""
    match Int32.TryParse toParse with
    | (true, x) -> Some x
    | (false, _) -> None

let splitByArrow (i: string) = i.Split([|" -> "|], StringSplitOptions.None)

let addTower (towers: Map<string, Tower>) (towerDef: string) = 
    let props = towerDef.Split [|' '|]
    match props with
    | [|name; Weight weight|] -> 
        let newTower = {Name = name; Weight = weight; Parent = None}
        (towers.Add(name, newTower), name)
    | _ -> (towers, "")

let addChildren (parent: string) (childrenMap: Map<string, string>)  (children: string) = 
    let addChildToMap (parent: string) (childrenMap: Map<string, string>) (child: string) =
        printfn "%s is parent to %s" parent child
        childrenMap.Add(child, parent)
    let folder = addChildToMap parent
    
    children.Split([|", "|], StringSplitOptions.None)
    |> Array.fold folder childrenMap

let buildState (state: State) (tower: string[]) =
    match tower with
    | [|towerDef; children|] -> 
        let (towers, parentName) = addTower state.towerMap towerDef
        let childrenMap = addChildren parentName state.childMap children
        {towerMap = towers; childMap = childrenMap}
    | [|towerDef|] ->
        let (towers, _) = addTower state.towerMap towerDef
        { state with towerMap = towers}
    | _ -> state

// let connectToParent (towerMap: Map<string, Tower>) (connection: string * string) = 
//     let (childKey, parentKey) = connection
//     let child = {towerMap.[childKey] with Parent = Some towerMap.[parentKey]}
//     printfn "%s has parent %s" child.Name child.Parent.Value.Name
//     towerMap.Add(childKey, child)

let rec findRoot (childMap: Map<string, string>) (child: string) =
    match childMap.ContainsKey child with
    | false -> child
    | true -> findRoot childMap childMap.[child]

let buildTree (state: State) = 
    // Map.toSeq state.childMap
    // |> Seq.fold connectToParent state.towerMap
    Map.pick (fun _ parent -> Some parent) state.childMap
    |> findRoot state.childMap

let main (input: string) = 
    let state = {towerMap = Map.empty; childMap = Map.empty}
    input.Split '\n'
    |> Array.map splitByArrow
    |> Array.fold buildState state
    |> buildTree
...
let finalTest = "navfz (187) -> jviwcde, wfwor, vpfabxa
adtfnx (74)
endsg (33)
