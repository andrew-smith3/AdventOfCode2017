...
let (|Int|_|) input = 
    ...
    | (true, int) -> Some(int)
    | _ -> 
        printfn "Int did not match: %s" input
        None
...
let (|Cond|_|) symbol = 
    ...
    | "<=" -> Some (<=)
    | x -> 
        printfn "Cond did not match: %s" x
        None
...
let (|Op|_|) symbol =
    ...
    | "dec" -> Some (-)
    | x -> 
        printfn "Op did not match: %s" x
        None

let getValueForRegister (registers: Map<string, int>) (reg: string) =
    match registers.ContainsKey reg with
    | true -> registers.[reg]
    | false -> 0

let checkCond (registers: Map<string, int>) (condition: string) =
    let cond = condition.Split [|' '|]
    match cond with
    | [|reg; Cond c; Int num|] -> 
        let regValue = getValueForRegister registers reg
        c regValue num
    | _ -> 
        printfn "Condition did not match: %s" condition
        false

let performOp (registers: Map<string, int>) (high: int) (operation: string) =
    let op = operation.Split [|' '|]
    match op with
    | [|reg; Op opFunc; Int num|] -> 
        let regValue = getValueForRegister registers reg
        let newValue = opFunc regValue num
        let registers = registers.Add(reg, newValue)
        if high > newValue then (registers, high) else (registers, newValue)
    | _ -> 
        printfn "Operation did not match: %s" operation
        (registers, high)
        

let folder (state: (Map<string, int> * int)) (instruction: string[]) =
    let (registers, high) = state
    match instruction with
    | [|op; cond;|] -> 
        match checkCond registers cond with
        | false -> (registers, high)
        | true -> performOp registers high op
    | _ ->  (registers, high)

let splitByIf (x: string) = x.Split([|" if "|], StringSplitOptions.None)

let main (input: string) = 
    let registers = Map.empty<string, int>
    input.Split [|'\n'|]
    |> Array.map splitByIf
    |> Array.fold folder (registers, 0) 
    |> snd
...
let finalTest = "b inc 580 if bkd > -1
v inc 491 if z >= -1
