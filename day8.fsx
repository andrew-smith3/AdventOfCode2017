...
let performOp (registers: Map<string, int>) (operation: string) =
    let op = operation.Split [|' '|]
    match op with
    | [|reg; Op o; Int num|] -> 
        let regValue = getValueForRegister registers reg
        registers.Add(reg, (o regValue num))
    | _ -> 
        printfn "Operation did not match: %s" operation
        registers
        

let folder (registers: Map<string, int>) (instruction: string[]) =
    match instruction with
    | [|op; cond;|] -> 
        match checkCond registers cond with
        | false -> registers
        | true -> performOp registers op
    | _ ->  registers
